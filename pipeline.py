...
class GAL:
	...
	def update_indices(self, selection_indices):
		if self.use_gnn:
			self.gnn_labeled_idx = np.concatenate((self.gnn_labeled_idx, [self.gnn_unlabeled_idx[e] + self.init_labeled_size for e in selection_indices]))
	...
	def _train_gnn(self):
		...
		for epoch in range(self.epochs):
			...
			loss = self.criterion(out[self.train_graph.train_mask], self.train_graph.y[self.train_graph.train_mask])  # Use only train mask nodes
	...
	def run(self, **kwargs):
		...
		for iter in iterations_progress:
			...
			if self.use_gnn:
				...
				iterations_progress.set_postfix({"Accuracy": accuracy, "LR test acc": LR_acc, "GNN Train Acc": gnn_train_acc, "GNN Test Acc": gnn_test_acc})
			else:
				accuracy = self._evaluate_model(self.classifier)
				accuracy_scores.append(accuracy)

				iterations_progress.set_postfix({"Accuracy": accuracy, "LR test acc": LR_acc})
