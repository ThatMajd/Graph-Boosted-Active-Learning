...
class GAL:
    def __init__(self,
        ...
        assert self.iterations * self.budget_per_iter <= len(dataset[
                                                                 "available_pool_labels"]), f"Not enough samples in pool ({self.iterations * self.budget_per_iter} > {len(dataset['available_pool_labels'])})"
    ...
    def create_train_graph(self, pytorch=True):
        ...
        pool_mask = np.array(
            [False] * (len(self.train_samples) + len(self.available_pool_samples) + len(self.test_samples)))
